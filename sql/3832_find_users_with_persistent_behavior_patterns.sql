...
numbered AS (
    SELECT
      ...
      action,
      action_date,
      action_date - ROW_NUMBER() OVER (PARTITION BY user_id, action ORDER BY action_date)::int AS grp
    FROM valid_days
),
streaks AS (
    SELECT user_id,
...
ranked AS (
    SELECT
      *,
      ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY streak_length, start_date ASC) AS rnk
    FROM streaks
)
SELECT 
  user_id,
