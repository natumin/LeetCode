...
pair_course AS (
    SELECT
      ...
      LEAD(course_name) OVER (PARTITION BY c1.user_id ORDER BY completion_date) AS second_course
    FROM course_completions AS c1
    INNER JOIN top_performing_students as p
    ON c1.user_id = p.user_id
)
SELECT
  first_course,
  second_course,
